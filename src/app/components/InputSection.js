...
const InputSection = ({ input, setInput, handleClick, error ,isGenerating,setIsGenerating }) => (
  <div className="input_section relative w-full h-screen">
    ...
    <div className="absolute top-0 left-0 w-full h-full z-20 flex flex-col justify-center items-center gap-5">
      ...
      {error && <p className="absolute bottom-56 text-white rounded-md px-3 py-1 text-xl font-bold">{error}</p>}
    </div>
  </div>
);
